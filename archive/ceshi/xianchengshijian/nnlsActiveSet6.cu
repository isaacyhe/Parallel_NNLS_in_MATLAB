...
__global__ void matVecMultiplyKernel(const double* mat, const double* vec, double* result, size_t rows, size_t cols) {
...
vector<double> matVecMultiplyCUDA(const vector<vector<double>>& mat, const vector<double>& vec) {
    ...
    CUDA_CHECK(cudaMemcpy(d_mat, flat_mat.data(), rows * cols * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_vec, vec.data(), cols * sizeof(double), cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaMemcpy(result.data(), d_result, rows * sizeof(double), cudaMemcpyDeviceToHost));
...
vector<vector<double>> transposeCUDA(const vector<vector<double>>& mat) {
    ...
    CUDA_CHECK(cudaMemcpy(flat_transposed.data(), d_transposed, rows * cols * sizeof(double), cudaMemcpyDeviceToHost));
    for (size_t j = 0; j < cols; ++j) {
        for (size_t i = 0; i < rows; ++i) {
        ...
        for (size_t j = 0; j <= i; ++j) {
            ...
            if (i == j) {
                if (diag <= 0) {
                    ...
                    return vector<double>(n, -1.0); // Return error indicator
                }
                L[i][j] = sqrt(diag);
            } else {
...
vector<double> nnlsActiveSetCUDA(const vector<vector<double>>& C, const vector<double>& d, double tol = 1e-6) {
    size_t m = C.size();
    size_t n = C[0].size();
    ...
    while (true) {
        ...
        while (true) {
            ...
            vector<vector<double>> C_active_T = transposeCUDA(C_active);
            ...
            if (x_active[0] == -1.0) { // Error from Cholesky

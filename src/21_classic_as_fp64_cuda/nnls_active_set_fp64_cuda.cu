...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    GPU_ALLOC(&d_C_sub, (size_t)m * n * sizeof(double));  // worst case m x n
    ...
    for (int outer_iter = 0; outer_iter < max_outer_iter; ++outer_iter) {
        ...
        CUBLAS_CHECK(cublasDgemv(cublas_handle, CUBLAS_OP_N, m, n,
        ...
        // Download gradient
        CUDA_CHECK(cudaMemcpy(gradient.data(), d_g, n * sizeof(double), cudaMemcpyDeviceToHost));
        ...
        for (int inner_iter = 0; inner_iter < max_inner_iter; ++inner_iter) {
            ...
            // Extract active columns via D2D copies (columns are contiguous)
            for (int i = 0; i < np; ++i) {
                int j = active_idx[i];
            ...
            // Solve R * z = (Q^T * d) via triangular solve (first np entries)
            double one_trsm = 1.0;
            CUBLAS_CHECK(cublasDtrsm(cublas_handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_UPPER,
                                     CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                                     np, 1, &one_trsm, d_C_sub, m, d_d_work, m));

            // Download result (first np entries)
            vector<double> x_active(np);
            CUDA_CHECK(cudaMemcpy(x_active.data(), d_d_work, np * sizeof(double),

function x = nnls_fast_nnls_st(C, d)
% NNLS_FAST_NNLS_ST  FAST-NNLS (Cobb et al. 2025) in pure MATLAB (single-threaded)
%
%   x = nnls_fast_nnls_st(C, d)
%
...
%     x - Solution vector (n x 1) with non-negative entries
    ...
    % Input validation
    if nargin ~= 2
        error('nnls_fast_nnls_st:input', 'Two inputs required: C, d');
    end
    [m, n] = size(C);
    if length(d) ~= m
        error('nnls_fast_nnls_st:dim', 'Dimension mismatch: rows(C) ~= length(d)');
    end
    ...
    while any(~P) && max(w(~P)) > tol
        ...
        while true
            ...
            if ~any(infeasible)
                % Fallback: use standard interpolation for any s <= 0
                infeasible = P & (s <= 0);
            end

...
vector<double> nnlsActiveSet(const vector<vector<double>>& C, const vector<double>& d, double tol = 1e-6) {
    ...
    while (true) {
        ...
        while (true) {
            ...
            vector<vector<double>> A(C_active[0].size(), vector<double>(C_active[0].size(), 0.0));
            for (size_t i = 0; i < C_active[0].size(); ++i) {
                for (size_t j = 0; j < C_active[0].size(); ++j) {
                    for (size_t k = 0; k < m; ++k) {
                        A[i][j] += C_active[k][i] * C_active[k][j];
                    }
                }
            }

            // 计算 C_active^T d
            vector<double> b(C_active[0].size(), 0.0);
            for (size_t i = 0; i < C_active[0].size(); ++i) {
                for (size_t k = 0; k < m; ++k) {
                    b[i] += C_active[k][i] * d[k];
                }
            }
